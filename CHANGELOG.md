...
- **订单** 新增 `aggregateDaily`（按日聚合、近 N 天连续升序、无数据日 0 填充）+ `trend(days)`（读 store 返回 `{days, data:[{date,amount,count}]}`）。
- **工作台** dashboard 新增「成交额趋势」区块：近 7 日订单成交额 → 内联 SVG 折线图（网格/面积渐变/数据点/数值/日标签，零依赖自适应）。
- 测试 `_biz_trend_test.js` **10 断言** → 全量 **109 套件 0 失败** + 四端同源 ✅。

### 2026-09-08 · 工作台聚合 · 业务概览

- **订单** `orders.dashStats` + **审批** `approvals.dashStats`（纯逻辑统计：订单总数/进行中/待处理/已完成/成交总额；审批总数/待审批/已通过/已驳回）。
- **工作台** dashboard 新增「业务概览」区块：订单卡（笔数 · 进行中 · 待处理 + 成交总额）与审批卡（待审批 · 通过 · 驳回），点击卡片跳转对应视图。
- 测试 `_biz_dashboard_test.js` **14 断言** → 全量 **108 套件 0 失败** + 四端同源 ✅。

### 2026-09-08 · 工作台聚合 · 库存预警

- **库存** 新增 `lowStock`（纯逻辑查询）：低于安全线或缺货品项聚合 `[{id,name,qty,safety,unit,severity(缺货/偏低)}]`，缺货优先排序。
- **工作台** dashboard 新增「库存预警」区块：命中时显示品项卡片（现有量/安全库存/缺货·偏低标签），点击卡片或「去库存处理」跳转库存视图；无命中自动隐藏。
- 测试 `_inventory_lowstock_test.js` **9 断言** → 全量 **107 套件 0 失败** + 四端同源 ✅。

### 2026-09-08 · 库存流水 导出 CSV

- **库存** 新增 `logCsvRows`（纯逻辑：流水→CSV 行，时间正序、入/出库 +/- 前缀、已撤销标记）+ `exportLogCsv`（读明细→按当前筛选导出，复用 `OS.export.csv`，UTF-8 BOM 兼容 Excel 中文）。
- 台账「流水」面板新增「导出 CSV」按钮，导出**当前筛选结果**（无筛选则导出全部流水）。
- 测试 `_inventory_log_export_test.js` **17 断言** → 全量 **106 套件 0 失败** + 四端同源 ✅。

### 2026-09-08 · 库存流水 按时间筛选 + 分页

- **库存** 新增 `filterLog`（纯逻辑：类型 入库/出库/全部 + 时间范围（开始/结束日期）+ 备注关键词，可组合）+ `pageLog`（纯逻辑：分页切片，页越界自动夹取、空集安全）。
- 台账「流水」面板新增筛选栏（类型下拉 / 开始·结束日期 / 关键词 + 筛选/重置）与分页（上一页/下一页 + 页码/总数），撤销后重填保持筛选与页码。
- 测试 `_inventory_log_page_test.js` **29 断言** → 全量 **105 套件 0 失败** + 四端同源 ✅。

### 2026-09-08 · 库存流水明细 + 撤销最近一笔

- **库存** 新增 `summarizeLog`（仅统计未撤销流水：累计入库/出库/有效笔数）+ `detailData`/`detail`（结构化详情含流水）。
- 新增 `undoAdjust`：**撤销最近一笔未撤销流水**（mark `revoked` + 追加反向记录，保留审计痕迹；撤销记录本身视为已撤销，避免可撤销振荡）。
- 台账「流水」按钮展开面板（当前库存 / 累计入库·出库 / 有效流水 + 明细表 + 「撤销最近一笔」按钮，撤销后重填保持展开）。
- 测试 `_inventory_log_test.js` **24 断言** → 全量 **104 套件 0 失败** + 四端同源 ✅。

### 2026-09-08 · 订单/审批详情展开 + 审批状态留痕

- **订单 / 审批** 列表新增「详情」按钮，点击展开详情面板并显示**状态流转时间线**（statusHistory：状态 / 时间 / 备注），可收起。
- **审批** 补 `statusHistory` 留痕：`update` 状态变更、`batchSetStatus` 批量流转均自动追加。
- 新增 `detailData`（结构化详情，纯逻辑）+ `detail(id)`（数据访问）；订单含金额/状态/备注/时间。
- 测试 `_biz_detail_test.js` **23 断言**（含 DOM 展开/收起）→ 全量 **103 套件 0 失败** + 四端同源 ✅。
