...
> 当前版本 **1.0.16**（tag `v1.0.16`）；以下代码已提交但**尚未发版**：AP / AQ / AR（随下次打包批量走）。
...
| git remote / push | ⛔ 待用户提供 URL | 无 remote，本地 17 个 tag 未推；给 github/gitee 类（代理可达）URL 即可推 |
| Android APK | ⛔ 环境无 SDK/gradle | 未伪造；`clients/android/app/build` 已移出 git 索引 |
| PDF→DOCX/TXT/MD/Excel 版面还原 | 🟡 边界 | 文本提取级，非像素级 |
