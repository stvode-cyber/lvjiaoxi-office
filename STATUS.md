...
| 09 业务板块 | —（并入 05 外壳） | ✅ 已实现 | 订单/库存/审批 本地 CRUD（IndexedDB）+ 我的（账户/偏好/关于）+ CSV 导出 + 批量操作（批量删除/审批批量通过·驳回）+ 库存出入库（数量调整 + 流水）+ 订单单条状态流转（下拉 + statusHistory 留痕） |
...
| 08 测试与质量 | [总纲](模块/08_测试与质量_总纲.md) | ✅ 已实现 | 102 套件 0 失败；E2E 已补（171 断言） |
