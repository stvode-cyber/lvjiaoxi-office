...
| 模块 | 总纲 | 整体状态 | 一句话 |
|------|------|----------|--------|
| 01 文档编辑 | [总纲](模块/01_文档编辑_总纲.md) | ✅ 核心已实现 | 五模块 + MindMap 可实跑；**OS.Versions 历史版本快照已接入 markDirty**（debounce 2s + 30s 间隔节流，每文档最多 50 版本），shell.js backstage 「历史版本」面板（list/diff/revert UI，LCS 行级 diff，回退自动存 pre-revert 备份） |
...
| 08 测试与质量 | [总纲](模块/08_测试与质量_总纲.md) | ✅ 已实现 | **95 套件 0 失败**（含 _versions_test 44 断言，v1.1.1 后续清理临时 release 测试 -3）；E2E 已补（171 断言） |
...
- **2026-09-18（v1.1.1 后续）OS.Versions 历史版本快照 UI 接入**：`app/js/versions.js` 已存在但未挂载 → `app/index.html` 加载 + `shell.js` markDirty 调 `OS.Versions.autoSnap(docId, stateGetter)`（debounce 2s + 30s 间隔节流）；shell.js backstage 加「历史版本」Tab：版本列表（时间/标签/原因/大小，倒序，最新标 TAG）+ 操作（**立即快照**/**对比**两版 LCS 行级 diff/**回退**自动存 pre-revert 备份/**删除**单版）+ diff 面板（增删统计 + 着色 diff-body）；CSS `.bs-hist-row/.bs-diff-*` 入 style.css；`versions.js` 暴露 `_lineDiff/_MAX_PER_DOC/_MAX_AUTO_INTERVAL` 供单测；`sw.js` 加 versions.js + CACHE v20→v21；同步 iOS/HarmonyOS（4 文件 × 2 端）；新增 `_versions_test.js` **44 断言**（LCS 纯函数 7 + quickDiff 1 + save/list/get/remove IDB shim 12 + diff 6 + revert pre-revert 备份 2 + autoSnap 节流 2 + clear 2 + 常数 2）；整链 **95 套件 0 失败** + 四端同源 ✅。**同日顺手发现**：handoff-2026-09-14.md 标为 P0 的「Presentation 加 pptx 导入」**实际已在 v1.0.20 实现**（`app/js/import-ooxml.js` parsePptx，含 slideMaster/Layout 背景继承、rId→media 解析、EMUscale → 像素、shapeText/runStyle 提取），handoff 信息过时无需重做。
