...
| 02 PDF | [总纲](模块/02_PDF_总纲.md) | 🟡 部分实现 | 阅读/合并拆分/批注(类型齐全+图层+AP图像/矢量)/表单/文本层/多格式导出/属性解析/书签目录/附件提取 ✅；PDF→Excel/PDF→DOCX 版面还原有限 |
...
| 08 测试与质量 | [总纲](模块/08_测试与质量_总纲.md) | ✅ 已实现 | 76 套件 0 失败；缺完整 E2E |
...
- **测试**：`npm test` 整链 **76 套件 0 失败**。
- **同源**：四端经 `sync-clients` 与 `app/` 一致 ✅。
- **最后更新**：2026-08-31（新增 AG=PDF 批注时间线（按天分组时序流 + 跳页 + 导出 MD），整链 76 套件 0 失败；S→AG 全特性已落地。另：Android APK 因本环境无 SDK 阻塞、正式发版打 tag 因本目录非 git 仓库需移至有工具链环境执行——本地已跑通 release:check + 重建 dist 安装包 + make-release dry-run）。
