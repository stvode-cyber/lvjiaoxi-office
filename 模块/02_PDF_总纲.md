...
| PDF 结构预检/完整性诊断（Preflight） | ✅ 已实现 | `OS.PdfPreflight` 只读体检 17 项（复用 OS.PdfTool._inflate/parseObjStm 展开 PDF 1.5+ 对象流）：PDF 头版本、%%EOF、startxref、xref 表/流、trailer /Root、Root 对象定义、/Catalog 类型、/Size 一致性、悬挂引用（仅扫字典部分，流二进制不参与）、重复对象定义、页面树环（**自带 visited 安全走树**——OS.PdfTool.parsePdf 的 walk 无环保护，遇环会栈溢出，故不依赖其 pages）、页数 0、流 /Length 缺失/间接引用/失配（±2 容差）、ObjStm 解压失败、孤儿对象；错误/警告/提示三级 + verdict(errors·warnings·pass)；面板(级别色条 + 对象/页数/引用统计) + 导出诊断报告(MD)；_pdf_preflight_test 42 断言 |
| PDF 文档历史/增量更新审计（History） | ✅ 已实现 | `OS.PdfHistory` 沿 trailer /Prev 链回溯每次保存版本（versionCount、逐版 xrefOffset/Size/Root/Info/viaStream），统计 %%EOF/startxref/xref 段计数，检测线性化(/Linearized)、/Prev 链断裂(chainBroken)、计数不一致(count-mismatch)；verdict single/multi/broken；面板版本表 + 导出历史报告(MD)；_pdf_history_test 39 断言 |
