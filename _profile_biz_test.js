...
function setup(over) {
  const dom = new JSDOM(`<!DOCTYPE html><html><head><meta name="x-app-version" content="1.0.16"></head><body><div id="pf"></div></body></html>`,
...
(async function main() {
  ...
  console.log("--- 1. 纯函数 ---");
  ok("appVersion 读取 meta", setup().pf.appVersion() === "1.0.16");
  ...
  console.log("\n--- 2. 已登录渲染 ---");
  {
    ...
    ok("含版本信息", el.innerHTML.includes("1.0.16"));
  }
