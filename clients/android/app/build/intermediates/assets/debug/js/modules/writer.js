...
(function (global) {
  ...
  function mount(host, doc, ctx) {
    ...
    async function exportAs(fmt) {
      ...
      if (fmt === "docx") {
        if (global.htmlDocx && global.htmlDocx.asBlob) {
          OS.util.download(global.htmlDocx.asBlob(wrapHtml(withCmt, title)), title + ".docx");
        } else {
          OS.toast("DOCX 引擎未加载（需联网），已降级导出 HTML", "warn");
          OS.util.download(new Blob([wrapHtml(withCmt, title)], { type: "text/html" }), title + ".html");
        }
        return;
      }
