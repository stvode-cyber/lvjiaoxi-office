...
(function (global) {
  ...
  function setProvider(cfg) {
    ...
    if (OS && OS.settings) OS.settings.set("aiUseProxy", false); // 手动 endpoint 优先
    return provider;
  }
  // 服务端同源代理：浏览器只调 /api/ai/chat，真实密钥留服务端（LVJX_AI_*）。
  // 依赖 OS.auth.current() 提供 Bearer token；未配置上游时服务器返回 501，run() 回退本地。
  function setProxyProvider(enabled, model) {
    const on = !!enabled;
    if (OS && OS.settings) OS.settings.set("aiUseProxy", on);
    if (on) {
      provider = {
    ...
    } else if (OS && OS.settings) {
      provider = OS.settings.get("aiProvider") || null;
    }
    return provider;
  }
  function isProxy() { return !!(provider && provider.proxy); }
  ...
  async function completeCloud({ system, user, onDelta, signal }) {
    ...
    if (!isProxy && provider.apiKey) headers["Authorization"] = "Bearer " + provider.apiKey;
    if (isProxy) {
      // 同源代理：附登录态 Bearer token（仅本机/同源，密钥不暴露前端）
      const sess = (OS.auth && OS.auth.current) ? OS.auth.current() : null;
    ...
    // 代理未配置上游：服务器回 501 ai-not-configured，诚实回退本地而非报错阻断
    if (resp.status === 501) throw Object.assign(new Error("ai-not-configured"), { code: 501 });
  ...
  async function run({ mode, system, user, onDelta, signal, allowCloud }) {
    ...
    if (provider && allowCloud !== false && !localOnly) {
      try {
        ...
        // no-provider / 代理未配置上游(501)：静默回退本地，不弹警告
        if (e.message !== "no-provider" && e.code !== 501)
          OS && OS.toast && OS.toast("云端调用失败，已切换本地引擎：" + e.message, "warn");
      }
    }
  ...
  OS.AI = {
    ...
    setProvider, getProvider, setProxyProvider, isProxy,
    MODES,
