...
- 工程交接：`.workbuddy/handoff-2026-09-08.md`（最新，v1.0.17 · 101 套件 0 失败 · 四端同源 · GitHub 已推 · 含业务五板块 + 测试环境修复 + 业务批量操作 + 库存出入库；卡点：① 发布到 VPS 待用户侧 ② iOS/HarmonyOS 编译）

---

## 三、当前基线（2026-09-08 更新）

- **101 套件 0 失败**（出入库后门禁）+ 四端同源 ✅ + GitHub 已推（`stvode-cyber/lvjiaoxi-office`，tag v1.0.17）
- 业务板块已上线：顶部导航五板块（工作台/订单/库存/审批/我的），订单/库存/审批本地 CRUD + CSV 导出 + **批量操作**（biz-common 共享工具条；订单/库存批量删除；审批批量通过/驳回/删除）+ **库存出入库**（planAdjust/adjustQty 数量调整 + history 流水）
