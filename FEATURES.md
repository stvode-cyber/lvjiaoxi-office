...
| 后台自动更新 | ✅ 已实现 | 启动后 30 分钟后台轮询发布清单；Web/PWA 经 Service Worker 静默接管、点「立即更新」刷新；桌面版经 IPC 比对远程清单、点「前往下载」；账户抽屉可手动「检查更新」 |
