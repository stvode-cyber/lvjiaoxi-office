...
| 能力 | 状态 | 说明 |
|---|---|---|
| **顶部主导航** | ✅ 已实现 | 工作台 / 订单 / 库存 / 审批 / 我的 五视图（`app/js/shell.js` `initTopNav`/`switchTopNav`/`renderTopPanel`），编辑态自动复位工作台高亮；工作台聚合：订单/审批「业务概览」统计卡（`dashStats`）+ 库存「低库存预警」卡片（`lowStock`）+ 订单「成交额趋势」内联 SVG 折线图（`trend`/`aggregateDaily`） |
| **订单板块** | ✅ 已实现 | `OS.biz.orders`：本地 CRUD（IndexedDB，doc type="order"）+ 状态机（待处理/进行中/已完成/已取消）+ 统计（成交额/数量/状态分布）+ 新建/列表/统计表单渲染 + `dashStats` 工作台概览（总数/进行中/待处理/已完成/成交总额）+ `aggregateDaily`/`trend` 成交额趋势（近 7 日，date/amount/count）；纯逻辑 `validateOrder`/`summarize`/`planStatus`/`setStatus`/`aggregateDaily` 与 DOM 分离；`_orders_biz_test` 25 断言 + `_order_status_test` 21 断言 + `_biz_dashboard_test` 14 断言 + `_biz_trend_test` 10 断言 |
| **库存板块** | ✅ 已实现 | `OS.biz.inventory`：商品库存台账 CRUD + 低库存预警 + 出入库记录 + 账内历史流水（累计入库·出库·有效笔数）+ 流水「撤销最近一笔」（mark `revoked` + 反向记录审计留痕、防振荡）+ 流水**按时间筛选 + 分页**（类型/日期范围/备注关键词，上一页·下一页）+ 流水**导出 CSV**（时间正序，入/出库 +/- 与已撤销标记，按当前筛选导出）+ **低库存/缺货聚合查询** `lowStock`（缺货优先，工作台预警区展示）；`_inventory_biz_test` / `_inventory_adjust_test` / `_inventory_log_test` / `_inventory_log_page_test` / `_inventory_log_export_test` / `_inventory_lowstock_test` |
| **审批板块** | ✅ 已实现 | `OS.biz.approvals`：待办/已办 CRUD + 通过·驳回流转 + **审批意见留痕**（`setStatus`：通过可填意见留空、驳回必填理由，历史 `statusHistory` 带 comment，详情时间线展示）+ `dashStats` 工作台概览（总数/待审批/已通过/已驳回）；`_approvals_biz_test`（库存+审批合计 30 断言）+ `_biz_dashboard_test` 14 断言 + `_approval_comment_test` 18 断言 |
