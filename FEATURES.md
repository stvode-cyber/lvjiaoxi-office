...
| **审批板块** | ✅ 已实现 | `OS.biz.approvals`：待办/已办 CRUD + 通过·驳回流转 + `dashStats` 工作台概览（总数/待审批/已通过/已驳回）；`_approvals_biz_test`（库存+审批合计 30 断言）+ `_biz_dashboard_test` 14 断言 |
