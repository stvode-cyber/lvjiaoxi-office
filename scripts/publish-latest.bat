...
set RC=%errorlevel%
if %RC%==0 (
  for /f "tokens=2 delims=:" %%v in ('findstr /b "version:" dist\latest.yml 2^>nul') do set VER=%%v
  set VER=%VER: =%
  if defined VER (
    echo [完成] %VER% 已发布，旧客户端将拉 latest.yml 走应用内自动更新。
  ) else (
    echo [完成] 已发布，旧客户端将拉 latest.yml 走应用内自动更新。
  )
) else (
