...
(async () => {
  ...
  ok("svg 含 transform:scale(2)", svg.includes("transform:scale(2)"));
  ...
  ok("默认可被子模块解析", (await PdfTool.parsePdf(b1)).pages.length === 1);
  ...
  ok("带 media → 可解析为 1 页", (await PdfTool.parsePdf(b2)).pages.length === 1);
  ...
  ok("多页 → 解析为 2 页", (await PdfTool.parsePdf(b3)).pages.length === 2);
